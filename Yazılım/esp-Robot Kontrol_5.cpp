...
#include <Arduino.h>
#include "WiFi.h"
#include <Wire.h> //PCA8695 için
#include <Adafruit_PWMServoDriver.h> //PCA8695 için
#include <PPMReader.h> //RC Kumanda için
...
IPAddress staticIP(192, 168, 16, 166);
IPAddress gateway(192, 168, 110, 141);
IPAddress subnet(255, 255, 255, 0);
IPAddress dns(192, 168, 110, 141);

WiFiServer wifiServer(60);
#endif

#ifdef NETWORK3
const char* ssid = "The_Force";
const char* password = "Atasoy6100";
...
unsigned int ch4ORT = (ch4MAX-ch4MIN)/2+ch4MIN;

char rcKumanda = 0;
char tersKinematik = 0;
...
void loop() {
    ...
    if (client) {
      while (client.connected()) {
        while (client.available()>0) {
          ...
          // ters kinematik tercihi

          if(buton[4] && buton[6]){tersKinematik=0;buton[4]=0;buton[6]=0;}
          if(buton[5] && buton[7]){tersKinematik=1;buton[5]=0;buton[7]=0;}
  ...
  */
          ...
          if(!tersKinematik){pwm.setPWM(12, 0, ch2SER1);pwm.setPWM(13, 0, ch3SER1);}
...
//--------------------------------------------------------------------------------------------------------------------------------------------------//
    ...
    {
    //ters kinematik tercihi
    if(ch4RAW<ch4ORT-100){tersKinematik=0;}
    if(ch4RAW>ch4ORT+100){tersKinematik=1;}
