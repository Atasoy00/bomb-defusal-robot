  ...
  if (client) {
    while (client.connected()) {
      while (client.available()>0) {
        ...
        move=1;
        //kavrama
        if(buton[0]){/*75*/ ch1SER=284;}
        if(buton[1]){/*50*/ ch1SER=197;}
        if(buton[2]){/*0*/  ch1SER=SERVOMIN+10;}
        if(buton[3]){/*100*/ch1SER=SERVOMAX-80;}
        ...
        //doner_servo
        if(buton[6]){/*sola_don*/ch5SER+=move;if(ch5SER>SERVOMAX){ch5SER=SERVOMAX;}}
        if(buton[7]){/*saga_don*/ch5SER-=move;if(ch5SER<SERVOMIN){ch5SER=SERVOMIN;}}

        //yukseklik
        if     (tusy == 1){/*yukarı*/ch2SER+=move;if(ch2SER>SERVOMAX){ch2SER=SERVOMAX;}}
        else if(tusy ==-1){/*asagı*/ ch2SER-=move;if(ch2SER<SERVOMIN){ch2SER=SERVOMIN;}}

        //derinlik
        if     (joy[3] ==-1){/*ileri*/ch3SER+=move;if(ch3SER>SERVOMAX){ch3SER=SERVOMAX;}}
        else if(joy[3] == 1){/*geri*/ ch3SER-=move;if(ch3SER<SERVOMIN){ch3SER=SERVOMIN;}}
...
/*
        ...
        x = map(ch3SER, SERVOMIN,SERVOMAX, 70,176);
        y = map(ch2SER, SERVOMIN,SERVOMAX, 70,176);
